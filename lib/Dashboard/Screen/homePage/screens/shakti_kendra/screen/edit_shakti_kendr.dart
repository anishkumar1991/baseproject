...
class _EditShaktiKendraScreenState extends State<EditShaktiKendraScreen> {
  ...
  apiCall() {
    if (widget.isEdit == true) {
      context.read<EditShaktiKendrCubit>().zilaSelected = widget.vidhanSabhaName ?? '';
      context.read<EditShaktiKendrCubit>().zilaId = widget.vidhanSabhaId ?? 0;
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                            ...
                            onTap: () async {
                              if (context.read<EditShaktiKendrCubit>().zilaSelected == '') {
                                ...
                                await showModalBottomSheet(
                                    ...
                                    builder: (builder) {
                                      return MandalBottomSheet(cubit: cubit, text: S.of(context).mandal);
                                    });
                              }
                            },
                            dense: true,
                      ...
                      BlocBuilder<EditShaktiKendrCubit, EditShaktiKendrState>(
                        builder: (context, state) {
                          ...
                          if (state is FatchDataBoothEditShaktiKendraState) {
                            ...
                            if (widget.isEdit == true) {
                              for (int i = 0; i < (cubit.boothData.data?.length ?? 0); i++) {
                                if (widget.boothNumber!.contains(int.parse(cubit.boothData.data?[i].number ?? ''))) {
                                  cubit.chekedValue.add(cubit.boothData.data![i]);
                                  cubit.selectedBooth.add(cubit.boothData.data![i].id!);
                                }
                              }
                            }
                      ...
                      BlocBuilder<EditShaktiKendrCubit, EditShaktiKendrState>(builder: (context, state) {
                        return Column(
                          children: [
              ...
              BlocBuilder<EditShaktiKendrCubit, EditShaktiKendrState>(
                builder: (context, state) {
                  if (state is LoadingEditAndCreateEditShaktiKendraState) {
                    EasyLoading.show();
                  } else if (state is FatchDataEditAndCreateEditShaktiKendraState) {
